<?php
    ...
    $transaction = '<RetailTransactionRequest xmlns:i="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://schemas.datacontract.org/2004/07/wapiGateway.Models">
        <C_ADDRESS>' . $_POST['AddressLine1'] . ' ' . $_POST['AddressLine2'] . '</C_ADDRESS>
        <C_CARDNUMBER>' . $_POST['CC'] . '</C_CARDNUMBER>
        <C_CVV>' . $_POST['CVV'] . '</C_CVV>
        <C_EXP>' . $_POST['ddlExp_month'] . '' . $_POST['ddLExp_year'] . '</C_EXP>
        ...
        <T_AUTH>' . $_POST['Auth'] . '</T_AUTH>
        <T_CARD_PRESENT>' . $_POST['CrdPrsnt'] . '</T_CARD_PRESENT>
        <T_CODE>1</T_CODE>
        <T_DEVICE_ID>A1B2C3</T_DEVICE_ID>
        <T_ORDERNUM>' . time() . '</T_ORDERNUM>
        <T_PARTIAL_AUTH>false</T_PARTIAL_AUTH>
        <T_TAX>0</T_TAX>
        <T_TERMINAL_NUMBER>0001</T_TERMINAL_NUMBER>
        <T_TRACKDATA>' . $_POST['TrkData'] . '</T_TRACKDATA>
        <T_TRACKDATA_FORMAT>' . $_POST['TrkFrmt'] . '</T_TRACKDATA_FORMAT>
        <T_UTI>' . $_POST['FirstName'] . ' ' . $_POST['LastName'] . '</T_UTI>
    ...
    $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://gateway.sagepayments.net/web_services/gateway/api/retailtransactions');
...
<html>
    ...
    <body>
        ...
        <?php
            ...
            echo 'Your order number is ' . $result->ResponseTokens->GatewayTransactionResponseToken->Value . '<br />';
